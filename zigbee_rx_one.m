...
%   pkt.bytesHat, pkt.psdu_no_fcs, pkt.fcs_rx, pkt.crc_ok и т.п.
    ...
    freq_correction = exp(-1j*2*pi*freqOffset.*n/Fs);
    rx(startPreamble:end) = rx(startPreamble:end) .* freq_correction;

    % Оценка и компенсация остаточной фазы на преамбуле.
    preamble_n = (0:length(rxPreambleSeg)-1).';
    rxPreambleFreqCorr = rxPreambleSeg .* exp(-1j*2*pi*freqOffset.*preamble_n/Fs);
